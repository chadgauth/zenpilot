...
def _create_radar_can_parser(car_fingerprint, tss1=False):
  ...
  elif Params().get_bool("dp_toyota_radar_acc_tss1_object_mode"):
    RADAR_A_MSGS = list(range(0x301, 0x318, 2))
  else:
    RADAR_A_MSGS = list(range(0x680, 0x686))

  msg_a_n = len(RADAR_A_MSGS)
  msg_b_n = len(RADAR_B_MSGS) if not tss1 else 0

  signals, checks = []
  if tss1:
    signals = list(zip(
        ['ID'] * msg_a_n + ['LONG_DIST'] * msg_a_n + ['LAT_DIST'] * msg_a_n + ['SPEED'] * msg_a_n +
        ['LAT_SPEED'] * msg_a_n,
        RADAR_A_MSGS * 5))
    checks = list(zip(RADAR_A_MSGS, [15] * msg_a_n))
  else:
    signals = list(zip(['LONG_DIST'] * msg_a_n + ['NEW_TRACK'] * msg_a_n + ['LAT_DIST'] * msg_a_n +
                       ['REL_SPEED'] * msg_a_n + ['VALID'] * msg_a_n + ['SCORE'] * ['DIST'] * msg_a_n + ['REL_SPEED'] * msg_a_n + ['VALID'] * msg_a_n + ['SCORE'] * msg_b_n, RADAR_A_MSGS + RADAR_B_MSGS))
...
class RadarInterface(RadarInterfaceBase):
  def __init__(self, CP):
    ...
    elif CP.carFingerprint not in RADAR_ACC_CAR_TSS1:
    ...
    is_tss1 = CP.carFingerprint in RADAR_ACC_CAR_TSS1

    self.rcp = _create_radar_can_parser(CP.carFingerprint, is_tss1)
    self.valid_cnt = {key: 0 for key in (range(0x3f) if is_tss1 else self.RADAR_A_MSGS)}
